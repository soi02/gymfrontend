...
export default function StepKeywordDynamic({ onNext, onBack }) {
  ...
  const [selectedIds, setSelectedIds] = useState([]); // keywordIds
  const MAX = 5;
  ...
  const toggle = (id) => {
    setSelectedIds(prev => {
      ...
      if (prev.length >= MAX) return prev; // 최대 5개
      return [...prev, id];
    });
  };

  const handleNext = () => {
    onNext({ keywordIds: selectedIds }); // 백엔드 규약
  };
  ...
  // const handleNext = () => {
  //  // 백엔드 규약: keywordIds만 전달
  //   onNext({ keywordIds: selectedKeywordIdList });
  // };
  ...
  return (
    <div className="challenge-create-page">
      ...
      <h2 className="challenge-step-question">
        <strong>{selectedIds.length > 0 ? `${selectedIds.length}개 선택됨 · ` : ''}키워드를 골라주세요</strong>
      </h2>
            ...
            <div className="step-keyword-list">
              {(cat.keywords || []).map(k => {
                ...
                return (
                  <button
                    key={k.keywordId}
                    className={`step-keyword-button ${active ? 'active' : ''}`}
      ...
      <button className="next-button" onClick={handleNext} disabled={selectedIds.length === 0}>
        다음
      </button>
    </div>
  );
