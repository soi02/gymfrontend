...
const BuddyChatRoom = () => {
  ...
  const [otherBuddyProfileImage, setOtherBuddyProfileImage] = useState(null); // 추가: 상대방 프로필 이미지 상태
  ...
  // ✅ 새로운 함수: 채팅 기록과 별개로 상대방 정보를 가져옴
  const fetchOtherBuddyInfo = async () => {
    try {
      const token = localStorage.getItem('token');
      if (!token) {
        console.error("토큰이 없어 상대방 정보를 불러올 수 없습니다.");
        return;
      }
      ...
      console.log("매칭 정보 API 응답:", res.data);
  ...
  // ✅ 기존 함수: 채팅 기록만 가져옴 (상대방 이름 설정 로직 삭제)
  ...
  useEffect(() => {
    ...
    const setupChatRoom = async () => {
      await fetchOtherBuddyInfo(); // ✨ 상대방 정보 먼저 호출
      await fetchChats();
          ...
          return (
            <React.Fragment key={index}>
              ...
              >
                ...
                {/* ✅ 수정: 상대방 프로필 이미지를 새로 가져온 데이터로 표시 */}
                {!isMyMessage && (
                  <img
                    src={getFullImageUrl(otherBuddyProfileImage)} // 'chat.senderProfileImageUrl' 대신 새로운 상태 사용
